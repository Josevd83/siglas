...
//                ->andWhere('oc.id = fc.cargo_id')
	...
	public function datosFuncionarioCargoUnidadActual($funcionario_id)
	{
		$q = Doctrine_Query::create()
                ->select('c.id,f.id, f.primer_nombre as fnombre, f.primer_apellido as fapellido, fc.*,u.nombre as unombre,(CASE WHEN f.sexo=\'M\' THEN ct.masculino WHEN f.sexo=\'F\' THEN ct.femenino END) as 				ctnombre, ct.nombre as cargo_tipo, u.nombre as unidad, f.ci as cedula')
                ->from('Funcionarios_Funcionario f')
                ->innerjoin('f.Funcionarios_FuncionarioCargo fc')
                ->innerjoin('fc.Organigrama_Cargo c')
                ->innerjoin('c.Organigrama_UnidadFuncional u')
