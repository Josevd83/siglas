...
<script language="JavaScript">
    ...
    function do_upload() {
        ...
        $("#error_pase").hide();
        $("#error_unidad").hide();
	$("#error_seguridad_persona_primer_nombre").hide();
	$("#error_seguridad_persona_primer_apellido").hide();
        
        var paseSeleccionado = $('#seguridad_ingreso_llave_ingreso_id').val();

	if ($('#salida').val()==0)
	{
		$("#error_verificar_salida").show();
		error = true;
	}

	if ($('#seguridad_persona_primer_nombre').val()=='')
	{
		$("#error_seguridad_persona_primer_nombre").show();
		error = true;
	}

	if ($('#seguridad_persona_primer_apellido').val()=='')
	{
		$("#error_seguridad_persona_primer_apellido").show();
		error = true;
	}
        ...
        if ($('#unidad_recibe').val()=='' && $('#autocomplete_seguridad_ingreso_list_funcionario').val()==''){
            $("#error_unidad").show();
            error = true;
        }

//verificar equipos
function verificar_equipos(error)
{

$.ajax({
                type: 'POST',
                dataType: 'html',
                url: '<?php echo sfConfig::get('sf_app_seguridad_url'); ?>ingresa/verificarEquipos',
                data: $('#form_nueva_visita').serialize(),
                success:function(data, textStatus){
			if(data!=""){
				$("#error_equipo_sin_egreso").html(data+' aún no ha sido egresado de la institución.').show();
				error = true;
			}else{
				if(error == false) {
				    $("#button_guardar").hide();
